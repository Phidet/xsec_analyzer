...
std::map<std::string, SampleInfo> sample_info_map = {
    ...
    {"MicroBooNE_CC1MuNp_XSec_2D_PmuCosmu_nu_MC", {"/uboone/data/users"
                                                   "/gardiner/respmat-test-new-Muon2D.root",
                                                   "/uboone/app/users/gardiner"
    ...
    {"MicroBooNE_CC1MuNp_XSec_2D_PpCosp_nu_MC", {"/uboone/data/users"
                                                 "/gardiner/respmat-test-new-Proton2D.root",
                                                 "/uboone/app/users/gardiner"
...
void test_unfolding_nuwro_with_generators()
{
    ...
    // std::cout << "DEBUG test_unfolding_nuwro_with_generators - Point 0" << std::endl;
    ...
    for (const auto &matrix_pair : matrix_map)
    {
        ...
        std::cout << "DEBUG result.err_prop_matrix_ dimensions: " << result.err_prop_matrix_->GetNrows() << "x" << result.err_prop_matrix_->GetNcols() << std::endl;
        std::cout << "DEBUG matrix_pair.second.get_matrix() dimensions: " << matrix_pair.second.get_matrix()->GetNrows() << "x" << matrix_pair.second.get_matrix()->GetNcols() << std::endl;
        std::cout << "DEBUG err_prop_tr dimensions: " << err_prop_tr.GetNrows() << "x" << err_prop_tr.GetNcols() << std::endl;

        // auto matrix_ptr = matrix_pair.second.get_matrix();
        auto pMatrix = matrix_pair.second.get_matrix();

        // Cut out the sideband bins from the covariance matrix here
        // result.err_prop_matrix_ only contains ordinary bins ...
        // ... so first check that its half the size of the matrix we're multiplying it by (wich might contain sideband bins) 
        if(2*result.err_prop_matrix_->GetNrows() == pMatrix->GetNrows())
        {
            TMatrixD matrix_cut(pMatrix->GetNrows() - num_sideband_reco_bins, pMatrix->GetNcols() - num_sideband_reco_bins);
            std::cout<<"DEBUG: Cutting out sideband bins from covariance matrix - num_sideband_reco_bins: "<<num_sideband_reco_bins<<std::endl;
            // ... then cut out the sideband bins from the matrix we're multiplying it by
            for (int i = 0; i < matrix_cut.GetNrows(); ++i)
            {
                for (int j = 0; j < matrix_cut.GetNcols(); ++j)
                {
                    matrix_cut(i, j) = pMatrix->operator()(i, j);
                }
            }
            std::cout<<"DEBUG: matrix_cut dimensions: "<<matrix_cut.GetNrows()<<"x"<<matrix_cut.GetNcols()<<std::endl;
            pMatrix.reset(new TMatrixD(matrix_cut));
            std::cout<<"DEBUG: 0 matrix dimensions: "<<pMatrix->GetNrows()<<"x"<<pMatrix->GetNcols()<<std::endl;
        }
        
        std::cout<<"DEBUG: matrix dimensions: "<<pMatrix->GetNrows()<<"x"<<pMatrix->GetNcols()<<std::endl;
        if (result.err_prop_matrix_->GetNrows() != pMatrix->GetNrows() && result.err_prop_matrix_->GetNcols() != pMatrix->GetNcols())
            throw std::runtime_error("ERROR: matrix_pair.second.get_matrix() and result.err_prop_matrix_ have different dimensions: " + std::to_string(pMatrix->GetNrows()) + "x" + std::to_string(pMatrix->GetNcols()) + " and " + std::to_string(result.err_prop_matrix_->GetNrows()) + "x" + std::to_string(result.err_prop_matrix_->GetNcols()));
        ...
        unfolded_cov_matrix_map[matrix_pair.first] = std::make_unique<TMatrixD>(
            *result.err_prop_matrix_, TMatrixD::EMatrixCreatorsOp2::kMult,
            TMatrixD(*pMatrix, TMatrixD::EMatrixCreatorsOp2::kMult, err_prop_tr));
    }
    ...
    if (USE_ADD_SMEAR)
        multiply_1d_hist_by_matrix(A_C, genie_cv_truth); // Can be directly multiplied for all slices because A_C is a block diagonal matrix
    ...
    auto generator_truth_map = get_true_events_nuisance(sample_info, conv_factor);

    // Print out the generator_truth_map keys
    std::cout << "DEBUG test_unfolding_nuwro_with_generators - Point 1" << std::endl;
    for (const auto &pair : generator_truth_map)
    {
        std::cout << "Key: " << pair.first << std::endl;
    }

    std::cout << "DEBUG test_unfolding_nuwro_with_generators - Point 2" << std::endl;
    ...
    std::cout << "DEBUG test_unfolding_nuwro_with_generators - Point 3" << std::endl;
    ...
    std::cout << "DEBUG test_unfolding_nuwro_with_generators - Point 4" << std::endl;
    ...
    for (size_t sl_idx = 0u; sl_idx < sb.slices_.size(); ++sl_idx)
    {
        ...
        std::cout << "DEBUG test_unfolding_nuwro_with_generators - Point 5" << std::endl;
        const auto &slice = sb.slices_.at(sl_idx);
        std::cout << "DEBUG test_unfolding_nuwro_with_generators - Point 6" << std::endl;
        ...
        for (const auto &uc_pair : unfolded_cov_matrix_map)
        {
            ...
            uc_ptr.reset(SliceHistogram::make_slice_histogram(*result.unfolded_signal_, slice, uc_matrix.get()));
        }
        ...
        for (int i = start; i <= stop; i++)
        {
            for (int j = start; j <= stop; j++)
            {
                ...
                if (using_fake_data)
                    fake_hist_2d_slice(i - start, j - start) = fake_data_univ->hist_2d_->GetBinContent(i + 1, j + 1);
        ...
        if (sl_idx == 8)
            var_count = 0; // TODO: Find a better way to do this

        std::cout << "DEBUG test_unfolding_nuwro_with_generators - Point 7" << std::endl;
        ...
        for (const auto &pair : slice_gen_map)
        {
            ...
            {
                other = slice_gen_map.at("Unfolded NuWro Reco");
            }
            ...
            double chi2_alt = slice_h->hist_->Chi2Test(other->hist_.get(), "UW CHI2");
        ...
        c1->Update();
        std::cout << "DEBUG test_unfolding_nuwro_with_generators - Point 8" << std::endl;
        ...
        std::cout << "DEBUG test_unfolding_nuwro_with_generators - Point 9" << std::endl;
        write_pgfplots_files(output_file_prefix, slice_hist_table, slice_params_table);
        std::cout << "DEBUG test_unfolding_nuwro_with_generators - Point 10" << std::endl;
    } // slices
    std::cout << "DEBUG test_unfolding_nuwro_with_generators - Point 11" << std::endl;
    return 0;
}
...
int main()
{
    ...
    test_unfolding_nuwro_with_generators();
