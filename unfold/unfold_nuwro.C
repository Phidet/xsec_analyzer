...
void unfold_nuwro()
{
    ...
    // Latest NuWRO version <----------------------------------- Latest fake data test version
    // inputFiles input{ // The bin definition used for the reco file ensures that CC1pi events with p_pi < 100MeV are not double counted + overflow bins have been removed from the bin and slice definitions
    ...
    // NuWRO fake data without NuWro uncertainty
    inputFiles input{ // The bin definition used for the reco file ensures that CC1pi events with p_pi < 100MeV are not double counted + overflow bins have been removed from the bin and slice definitions
        "/exp/uboone/data/users/jdetje/ubcc1pi_univmake/22Feb24/univmake_output_nuwro_run1234bcd5_17Oct24_testingOnly_lowPiMomThreshold_allUncertainties_fixedBackground_mergedOverflow_containedMuXSec.root",
        "../nuwro_file_properties_testingOnly_lowPiMomThreshold_allUncertainties.txt",
        "../systcalc_unfold_fd_noNuWro.conf",
        "../ubcc1pi_neutral_slice_config_mergedOverflow_noSuperscripts.txt",
        "_nuwro_fixedBackground_mergedOverflow_containedMuXSec_noNuWro"
    };


    // Genie FSI fix closure test <----------------------------------- GENIE FSI fix
    // inputFiles input{ // The bin definition used for the reco file ensures that CC1pi events with p_pi < 100MeV are not double counted + overflow bins have been removed from the bin and slice definitions
    //     "/exp/uboone/data/users/jdetje/ubcc1pi_univmake/22Feb24/univmake_output_genieFSIFix_V2.root",
    //     "../GeniePionFSIFix_closure_file_properties.txt",
    //     "../systcalc_unfold_fd_noNuWro.conf",
    //     "../ubcc1pi_neutral_slice_config_mergedOverflow_noSuperscripts.txt",
    //     "_GeniePionFSIFix_noNuWro"
    // };

    // Make unfolded fractional uncertainty plots (NuWro fake date)
    ...
    // Make unfolded fractional uncertainty plots (Closure test)   
    std::vector< std::string > cov_mat_keys_total = { "total", "MCstats", "BNBstats"};
    std::vector< std::string > cov_mat_keys_xsec = {};
    ...
    std::cout<<"DEBUG unfold_nuwro() Point 1"<<std::endl;
    ...
    input.nameExtension += "_noGenerators";
    ...
    for (size_t sl_idx = 0u; sl_idx < sb.slices_.size(); ++sl_idx)
    {
        ...
        std::cout<<"DEBUG unfold_nuwro() Point 6.02"<<std::endl;
        if (std::find(cov_mat_keys_total.begin(), cov_mat_keys_total.end(), "xsec_total") != cov_mat_keys_total.end())
        {
        ...
        std::cout<<"DEBUG unfold_nuwro() Point 6.1"<<std::endl;
        ...
        std::cout<<"DEBUG unfold_nuwro() Point 6.2"<<std::endl;
        ...
        std::cout<<"DEBUG unfold_nuwro() Point 6.3"<<std::endl;
        ...
        std::cout<<"DEBUG unfold_nuwro() Point 12"<<std::endl;
        ...
        std::cout<<"DEBUG unfold_nuwro() Point 13"<<std::endl;
        ...
        std::cout<<"DEBUG unfold_nuwro() Point 14"<<std::endl;
        ...
        {
            const auto &name = pair.first;
            if( plotOnlyDataSample && name != "Unfolded Selection") continue;
            const auto *slice_h = pair.second;
            ...
            {
                oss << "#splitline{";
                if (name == "NuWro Truth") oss << "Fake Data Truth";
        ...
        std::cout<<"DEBUG unfold_nuwro() Point 15"<<std::endl;
        ...
        latex.DrawLatexNDC(0.3, 0.94, "MicroBooNE Simulation"); // Adjust the position (NDC coordinates)
        ...
        std::cout<<"DEBUG unfold_nuwro() Point 16"<<std::endl;
    } // slices
