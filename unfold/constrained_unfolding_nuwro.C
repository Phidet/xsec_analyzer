...
void constrained_unfolding_nuwro()
{
    ...
    // std::cout << "DEBUG constrained_unfolding_nuwro - Point 0" << std::endl;
    ...
    // const std::string respmat_file_name("/uboone/data/users/jdetje/ubcc1pi_univmake/100Percent_9/univmake_output_nuwro_6Dec23.root");
    ...
    const std::string respmat_file_name("/uboone/data/users/jdetje/ubcc1pi_univmake/100Percent_9/univmake_output_nuwro_with_sideband_noOverflow_noGolden_run1_13Jan23.root");
    // Do the systematics calculations in preparation for unfolding
    const auto *syst_constr_ptr = new ConstrainedCalculator(respmat_file_name, "../systcalc_unfold_fd_min.conf");
    const auto &syst_constr = *syst_constr_ptr;
    ...
    TH1D *genie_cv_truth = syst_constr.cv_universe().hist_true_.get();
    ...
    const auto &fake_data_univ = syst_constr.fake_data_universe();
    ...
    auto num_ordinary_reco_bins = getBinCount(syst_constr.reco_bins_, kSidebandRecoBin, false);
    auto num_sideband_reco_bins = syst_constr.reco_bins_.size() - num_ordinary_reco_bins;
    auto num_true_signal_bins = getBinCount(syst_constr.true_bins_, kSignalTrueBin, true);
    ...
    std::cout<<"DEBUG constrained_unfolding_nuwro - Point 1"<<std::endl;
    const auto &matrix_map_constr = *matrix_map_constr_ptr;
    const auto *cov_mat_constr = matrix_map_constr.at("total").cov_matrix_.get();

    std::cout<<"DEBUG constrained_unfolding_nuwro - Point 2"<<std::endl;
    const auto *matrix_map_ptr = syst.get_covariances().release();
    std::cout<<"DEBUG constrained_unfolding_nuwro - Point 3"<<std::endl;
    const auto &matrix_map = *matrix_map_ptr;
    const auto *cov_mat = matrix_map.at("total").cov_matrix_.get();
    std::cout<<"DEBUG constrained_unfolding_nuwro - Point 4"<<std::endl;


    std::cout << "cov_mat_constr dimensions: " << cov_mat_constr->GetNbinsX() << " x " << cov_mat_constr->GetNbinsY() << std::endl;
    std::cout << "cov_mat dimensions: " << cov_mat->GetNbinsX() << " x " << cov_mat->GetNbinsY() << std::endl;
    ...
    for ( int a = 1; a <= num_ordinary_reco_bins; ++a ) {
        ...
        double cm_constr = cov_mat_constr->GetBinContent( a, b );
        if ( cm != cm_constr ) throw std::runtime_error( "Covariance matrices is not the same for ordinary reco bins" );
        }
    }
    ...
    //     double cm_constr = cov_mat_constr->GetBinContent( a, b );
    //     if ( cm != cm_constr ) throw std::runtime_error( "Covariance matrices is not the same for sideband reco bins" );
    //     }
    // }
    ...
    TMatrixD* corr = new TMatrixD(util::CovarianceMatrixToCorrelationMatrix(util::TH2DToTMatrixD(*cov_mat_constr)));
    ...
    cm2->SaveAs("plots/plot_slice_entire_corr_nuwro_constr.pdf");
    ...
    auto result = unfolder->unfold(syst_constr);
    ...
    for (const auto &matrix_pair : matrix_map_constr)
    {
        ...
        // Print the dimensions of the matrices in the pair
        std::cout << "DEBUG matrix_pair: " << matrix_pair.second.get_matrix()->GetNrows() << " x " << matrix_pair.second.get_matrix()->GetNcols() << std::endl;
        std::cout << "DEBUG err_prop_tr: " << err_prop_tr.GetNrows() << " x " << err_prop_tr.GetNcols() << std::endl;
    }
    ...
    NormShapeCovMatrix bd_ns_covmat = make_block_diagonal_norm_shape_covmat(
        *result.unfolded_signal_, *result.cov_matrix_, syst_constr.true_bins_);
    ...
    cm1->SaveAs("plots/plot_unfolded_slice_entire_corr_nuwro_constr.pdf");
    ...
    c_ac->SaveAs("plots/plot_entire_additional_smearing_matrix_nuwro_constr.pdf");
    ...
    double total_pot = syst_constr.total_bnb_data_pot_;
    ...
    const auto cv_hist_2d = syst_constr.get_cv_hist_2d();
    ...
    c_cv_hist_2d->SaveAs("plots/entire_cv_hist_2d_nuwro_constr.pdf");

    for (size_t sl_idx = 0u; sl_idx < sb.slices_.size(); ++sl_idx)
    {
        // if (sl_idx != 4)
        //     continue; // TODO: remove !!!!!!!!!!!!!!!!!!!!!
        std::cout << "DEBUG constrained_unfolding_nuwro - Point 5" << std::endl;
        const auto &slice = sb.slices_.at(sl_idx);
        std::cout << "DEBUG constrained_unfolding_nuwro - Point 6" << std::endl;
        ...
        c_conf_1->SaveAs(("plots/cv_confusion_matrix_slice_" + std::string(sl_idx < 10 ? "0" : "") + std::to_string(sl_idx) + "_nuwro_constr.pdf").c_str());
        ...
        if(using_fake_data)
        {
            ...
            c_conf_fake->SaveAs(("plots/fake_confusion_matrix_slice_" + std::string(sl_idx < 10 ? "0" : "") + std::to_string(sl_idx) + "_nuwro_constr.pdf").c_str());
        }
        ...
        TColor::CreateGradientColorTable(nColors, stops, red, green, blue, 20);
        c_ac_slice->SaveAs(("plots/additional_smearing_matrix_slice_" + std::string(sl_idx < 10 ? "0" : "") + std::to_string(sl_idx) + "_nuwro_constr.pdf").c_str());
        ...
        c_corr_unf_slice->SaveAs(("plots/unfolded_correlation_matrix_slice_" + std::string(sl_idx < 10 ? "0" : "") + std::to_string(sl_idx) + "_nuwro_constr.pdf").c_str());
        ...
        c_corr_slice->SaveAs(("plots/correlation_matrix_slice_" + std::string(sl_idx < 10 ? "0" : "") + std::to_string(sl_idx) + "_nuwro_constr.pdf").c_str());
        ...
        std::cout << "DEBUG constrained_unfolding_nuwro - Point 7" << std::endl;
        ...
        for (const auto &pair : slice_gen_map)
        {
            ...
            std::cout<<"DEBUG constrained_unfolding_nuwro - Point 8" << std::endl;
            const auto &chi2_result = chi2_map[name] = slice_h->get_chi2(*slice_gen_map.at("Unfolded NuWro Reco"));
            std::cout<<"DEBUG constrained_unfolding_nuwro - Point 9" << std::endl;
        ...
        std::cout<<"DEBUG constrained_unfolding_nuwro - Point 10" << std::endl;
        double chi2_1 = slice_unf->hist_->Chi2Test(slice_cv->hist_.get(), "WU CHI2");
        std::cout<<"DEBUG constrained_unfolding_nuwro - Point 11" << std::endl;
        ...
        if (using_fake_data)
        {
            std::cout<<"DEBUG constrained_unfolding_nuwro - Point 12" << std::endl;
            ...
            std::cout<<"DEBUG constrained_unfolding_nuwro - Point 13" << std::endl;
        ...
        std::string out_pdf_name = "plots/plot_unfolded_slice_" + std::string(sl_idx < 10 ? "0" : "") + std::to_string(sl_idx) + "_nuwro_constr.pdf";
        ...
        // std::string out_pdf_name_cov = "plots/plot_slice_cov_" + std::string(sl_idx < 10 ? "0" : "") + std::to_string(sl_idx) + "_constr.pdf";
        ...
        // std::string out_pdf_name_unf_cov = "plots/plot_unfolded_slice_cov_" + std::string(sl_idx < 10 ? "0" : "") + std::to_string(sl_idx) + "_constr.pdf";
