...
void test_ubcc1pi_unfolding() {
    ...
    auto* syst_ptr = new MCC9SystematicsCalculator(
        "/uboone/data/users/jdetje/ubcc1pi_univmake/100Percent_NuWro/univmake_output.root",
        "../systcalc.conf" );

    std::cout<<"DEBUG Point U-2"<<std::endl;
    // Count the number of true bins in which signal events are stored
    int num_true_signal_bins = 0;
    for ( int t = 0; t < syst_ptr->true_bins_.size(); ++t ) {
        const auto& tbin = syst_ptr->true_bins_.at( t );
    ...
    TH1D* genie_cv_truth = syst_ptr->cv_universe().hist_true_.get();
    ...
    std::cout<<"DEBUG Point U-1"<<std::endl;
    ...
    const auto& fake_data_univ = syst_ptr->fake_data_universe();
    ...
    std::cout<<"DEBUG Point U-0.9"<<std::endl;
    ...
    auto& cov_mat_map = *syst_ptr->get_covariances().release();

    // Cout all the keys of the covariance matrix map
    std::cout << "DEBUG Covariance matrix types:\n";
    for ( const auto& pair : cov_mat_map ) {
        std::cout << "  " << pair.first << '\n';
    }

    std::cout<<"DEBUG Point U-0.8"<<std::endl;
    ...
    std::cout<<"DEBUG Point U0"<<std::endl;
  ...
  for ( const auto& matrix_pair : cov_mat_map ) {
  ...
  NormShapeCovMatrix bd_ns_covmat = make_block_diagonal_norm_shape_covmat(
    *result.unfolded_signal_, *result.cov_matrix_, syst_ptr->true_bins_ );
  ...
  unfolded_cov_matrix_map[ "total_blockwise_mixed" ]
    ...
    std::cout<<"DEBUG Point U3"<<std::endl;
    ...
    auto* sb = new SliceBinning( "../ubcc1pi_true_slice_config.txt" );
    ...
    std::cout<<"DEBUG Point U4"<<std::endl;
    ...
    std::cout<<"DEBUG Point U4.3"<<std::endl;
    ...
    std::cout<<"DEBUG Point U5"<<std::endl;
    ...
    std::cout<<"DEBUG Point U6"<<std::endl;
    ...
    std::cout<<"DEBUG Point U7"<<std::endl;
    ...
    std::cout<<"DEBUG Point U8"<<std::endl;
    ...
    for ( const auto& pair : slice_gen_map ) {
        ...
        std::cout<<"DEBUG Point U8.1 - name: "<<name<<std::endl;
        ...
        std::cout<<"DEBUG Point U8.2"<<std::endl;
    ...
    std::cout<<"DEBUG Point U8.3"<<std::endl;
    ...
    std::cout<<"DEBUG Point U8.4"<<std::endl;

    double ymax = -DBL_MAX;
    slice_unf->hist_->Draw( "e" );
    for ( const auto& pair : slice_gen_map ) {
        const auto& name = pair.first;
        std::cout<<"DEBUG Point U8.5 - name: "<<name<<std::endl;
        const auto* slice_h = pair.second;
        ...
        std::cout<<"DEBUG Point U8.6"<<std::endl;
    }
    ...
    std::cout<<"DEBUG Point U8.7"<<std::endl;
    if ( using_fake_data ) {
    ...
    for ( const auto& pair : slice_gen_map ) {
        const auto& name = pair.first;
        std::cout<<"DEBUG Point U8.8 - name: "<<name<<std::endl;
        const auto* slice_h = pair.second;

        std::string label = name;
        std::ostringstream oss;
        std::cout<<"DEBUG Point U8.81"<<std::endl;
        const auto& chi2_result = chi2_map.at( name );
        std::cout<<"DEBUG Point U8.82"<<std::endl;
        oss << std::setprecision( 3 ) << chi2_result.chi2_ << " / "
        << chi2_result.num_bins_ << " bin";
        if ( chi2_result.num_bins_ > 1 ) oss << 's';
        std::cout<<"DEBUG Point U8.83"<<std::endl;
        if ( name != "unfolded data" ) {
        label += ": #chi^{2} = " + oss.str();
        }

        lg->AddEntry( slice_h->hist_.get(), label.c_str(), "l" );
        std::cout<<"DEBUG Point U8.9"<<std::endl;
    }

    lg->Draw( "same" );
    std::cout<<"DEBUG Point U9"<<std::endl;
