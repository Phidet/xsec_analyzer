			...
			if (count_other == 0 && count_13 == 1 && count_211 == 1) {
				...
				for (size_t i = 0; i < mc_pdg->size(); i++) {
					if (std::abs((*mc_pdg)[i]) == 211) {
						...
						// if ((*mc_completeness)[i] > max_mc_completeness) {
						// 	max_mc_completeness = (*mc_completeness)[i];
						// 	true_pion_index = i;
						// }
						if(true_pion_index != -1) throw std::runtime_error("More than one pion found in mc_pdg");
						true_pion_index = i;
					}
				}
				...
				if (true_pion_index != -1) {
					...
					purity = (*mc_purity)[true_pion_index];
					completeness = (*mc_completeness)[true_pion_index];
				}
				else
				{
