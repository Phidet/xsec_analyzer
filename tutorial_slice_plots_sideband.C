...
void slice_plots(const bool normaliseByBinWidth) {
  ...
  std::cout<<"DEBUG tutorial_slice_plots_sideband Point 0"<<std::endl;
  #ifdef USE_FAKE_DATA
    // Initialize the FilePropertiesManager and tell it to treat the NuWro
    // MC ntuples as if they were data
    auto& fpm = FilePropertiesManager::Instance();
    fpm.load_file_properties( "nuwro_file_properties.txt" );
    auto* syst_ptr = new MCC9SystematicsCalculator("/uboone/data/users/jdetje/ubcc1pi_univmake/univmake_output_nuwro_V8_withEXT_sideband.root", "systcalc_unfold_fd_min.conf" );
    std::string nameExtension = "_side_fd";
  #else
    auto& fpm = FilePropertiesManager::Instance();
    fpm.load_file_properties( "file_properties_123.txt" );
    auto* syst_ptr = new MCC9SystematicsCalculator(
    "/pnfs/uboone/persistent/users/jdetje/univmakeOut/univmake_output_bnb_sideband_4Dec23.root",
    "systcalc.conf" );
    std::string nameExtension = "_side_bnb";
  #endif

  std::cout<<"DEBUG tutorial_slice_plots_sideband Point 1"<<std::endl;
  auto& syst = *syst_ptr;
  ...
  std::cout<<"DEBUG tutorial_slice_plots_sideband Point 2"<<std::endl;
  ...
  std::cout<<"DEBUG tutorial_slice_plots_sideband Point 3"<<std::endl;

  for ( size_t sl_idx = 0u; sl_idx < sb.slices_.size(); ++sl_idx ) {
    std::cout<<"DEBUG tutorial_slice_plots_sideband Point 3.1 sl_idx: "<<sl_idx<<std::endl;
    if(sl_idx>2 && sl_idx<8) continue;

    const auto& slice = sb.slices_.at( sl_idx );
    std::cout<<"DEBUG tutorial_slice_plots_sideband Point 3.2"<<std::endl;
    ...
    std::cout<<"DEBUG tutorial_slice_plots_sideband Point 3.3"<<std::endl;
    ...
    std::cout<<"DEBUG tutorial_slice_plots_sideband Point 4"<<std::endl;
    ...
    std::cout<<"DEBUG tutorial_slice_plots_sideband Point 5"<<std::endl;
    ...
    slice_bnb->hist_->SetTitle("Selected #nu_{#mu}CC0#piXp, X #geq 1 Events");
    ...
    std::cout<<"DEBUG tutorial_slice_plots_sideband Point 6"<<std::endl;
    ...
    std::cout<<"DEBUG tutorial_slice_plots_sideband Point 7"<<std::endl;

    TCanvas* c2 = new TCanvas;
    // TLegend* lg2 = new TLegend( 0.7, 0.7, 0.9, 0.9 );
    TLegend* lg2 = new TLegend( 0.2, 0.3);

    std::cout<<"DEBUG tutorial_slice_plots_sideband Point 7.1"<<std::endl;

    auto* total_frac_err_hist = frac_uncertainty_hists.at( "total" );
    std::cout<<"DEBUG tutorial_slice_plots_sideband Point 7.2"<<std::endl;
    ...
    total_frac_err_hist->SetTitle("Fractional Uncertainty of Selected #nu_{#mu}CC0#piXp, X #geq 1 Events");
    total_frac_err_hist->GetYaxis()->SetTitle("Fractional Uncertainty");

    std::cout<<"DEBUG tutorial_slice_plots_sideband Point 7.3"<<std::endl;
    ...
    for ( auto& pair : frac_uncertainty_hists ) {
      std::cout<<"DEBUG tutorial_slice_plots_sideband Point 7.4"<<std::endl;
      ...
      std::cout<<"DEBUG tutorial_slice_plots_sideband Point 7.5"<<std::endl;
  ...
  std::cout<<"DEBUG tutorial_slice_plots_sideband Point 8"<<std::endl;

}

int tutorial_slice_plots_sideband() {
  slice_plots(true);
  return 0;
}
