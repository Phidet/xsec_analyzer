...
#define FILE_PROPERTIES "nuwro_file_properties_testingOnly_lowPiMomThreshold.txt"
...
{
    std::cout<<"DEBUG NuWroCutPlots Point X0"<<std::endl;
    ...
    std::cout<<"DEBUG NuWroCutPlots Point X1"<<std::endl;
    ...
    constexpr std::array< NFT, 4 > file_types = { NFT::kOnBNB, NFT::kNumuMC }; // NFT::kIntrinsicNueMC // NFT::kExtBNB

    // Similar array that includes only the CV MC samples
    constexpr std::array< NFT, 2 > mc_file_types = { NFT::kNumuMC}; // NFT::kIntrinsicNueMC // NFT::kDirtMC
    ...
    std::cout<<"DEBUG NuWroCutPlots Point X2"<<std::endl;
    ...
    for ( const int& run : runs ) {
      ...
      for ( const auto& type : file_types ) {
        ...
        std::cout<<"DEBUG NuWroCutPlots Point X3.2.2"<<std::endl;

        for ( const auto& file_name : ntuple_files ) {
          std::cout<<"DEBUG NuWroCutPlots Point X3.3"<<std::endl;
    ...
    std::cout<<"DEBUG NuWroCutPlots Point X4"<<std::endl;
    ...
    std::cout<<"DEBUG NuWroCutPlots Point X5"<<std::endl;
    ...
    std::cout<<"DEBUG NuWroCutPlots Point X6"<<std::endl;
    ...
    std::cout<<"DEBUG NuWroCutPlots Point X7"<<std::endl;
    ...
    std::string plot_name = "plots/" + out_name + "_runs" + runNames + "_testingOnly_lowPiMomThreshold_containedMuonOption";
    c1->SaveAs( (plot_name + ".pdf").c_str() );
    c1->SaveAs( (plot_name + ".png").c_str() );
    c1->SaveAs( (plot_name + ".C").c_str() );
    
    std::cout<<"DEBUG NuWroCutPlots Point X8"<<std::endl;
}
...
void NuWroCutPlots() {
  ...
  const std::map<std::string, std::vector<double>> binEdges{
      ...
      {"muonMomentum2", get_bin_low_edges( 0.15, 1.5, 30 )},
      {"pionMomentum", get_bin_low_edges( 0, 1.0, 30 )},
      {"pionMomentum2", get_bin_low_edges( 0, 0.2, 30 )},
  };


  for (const auto& runSet : runSets)
  {
    std::cout << "\033[31mWARNING: Code is not correct for fake data when plots are not restricted to signal events!\033[0m" << std::endl;

    make_plots( /* out_name = */          "NuWroCutPlot_TruePionMomentum",
                /* branchexpr = */        "cc1pi_truth_pionMomentum",
                /* selection = */         "cc1pi_signal && cc1pi_selected_generic",
                /* runs = */              runSet,
                /* bin_low_edges = */     binEdges.at("pionMomentum2"),
                /* x_axis_label = */      "True pion momentum [GeV/c]",
                /* y_axis_label = */      "# Events", 
                /* title = */             "True pion momentum of selected signal events",
                /* vertical_lines = */    std::set<float>{0.1f} );

    std::cout<<"DEBUG NuWroCutPlots Point 1"<<std::endl;

    make_plots( /* out_name = */          "NuWroCutPlot_RecoPionMomentum",
                /* branchexpr = */        "cc1pi_reco_pionMomentum",
                /* selection = */         "cc1pi_signal && cc1pi_selected_generic",
                /* runs = */              runSet,
                /* bin_low_edges = */     binEdges.at("pionMomentum2"),
                /* x_axis_label = */      "Reco pion momentum [GeV/c]",
                /* y_axis_label = */      "# Events", 
                /* title = */             "Reco pion momentum of selected signal events",
                /* vertical_lines = */    std::set<float>{0.1f} );
  }
}