...
void slice_plots(const bool normaliseByBinWidth) {
  ...
  std::cout<<"DEBUG tutorial_slice_plots Point 0"<<std::endl;
    ...
    fpm.load_file_properties( "nuwro_detvar_file_properties.txt" );
    auto* syst_ptr = new MCC9SystematicsCalculator(
      "/uboone/data/users/jdetje/ubcc1pi_univmake/100Percent_10/univmake_detvar_output_nuwro_with_sideband_overflow_all_22Jan23.root",
      "systcalc_unfold_fd_modified_detvar.conf" );
    std::string nameExtension = "_fd_detvar";
  #else
    // auto& fpm = FilePropertiesManager::Instance();
    // fpm.load_file_properties( "file_properties_123.txt" );
    // auto* syst_ptr = new MCC9SystematicsCalculator(
    // "/uboone/data/users/jdetje/ubcc1pi_univmake/100Percent_9/univmake_output_bnb_5Dec23_123.root",
    // "systcalc_modified.conf" );
    // std::string nameExtension = "_bnb_detvar";
  #endif
  ...
  for ( size_t sl_idx = 0u; sl_idx < sb.slices_.size(); ++sl_idx ) {
    ...
    std::vector< std::string > cov_mat_keys = { "total", "detVarLYatten", "detVarLYdown", "detVarLYrayl", "detVarRecomb2", "detVarSCE", "detVarWMAngleXZ", "detVarWMAngleYZ", "detVarWMX", "detVarWMYZ"};
    #ifdef USE_FAKE_DATA
    // cov_mat_keys = { "total", "xsec_total", "MCstats", "EXTstats", "BNBstats"};
    cov_mat_keys = { "total", "detVarLYatten", "detVarLYdown", "detVarLYrayl", "detVarRecomb2", "detVarSCE", "detVarWMAngleXZ", "detVarWMAngleYZ", "detVarWMX", "detVarWMYZ"};
...
int tutorial_slice_plots_bnb_detvar() {
  slice_plots(true);
  return 0;
}
