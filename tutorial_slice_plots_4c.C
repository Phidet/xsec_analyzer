...
void slice_plots(const bool normaliseByBinWidth) {
  ...
  auto& fpm = FilePropertiesManager::Instance();
  fpm.load_file_properties( "file_properties_4c.txt" );

  auto* syst_ptr = new MCC9SystematicsCalculator(
    "/uboone/data/users/jdetje/ubcc1pi_univmake/100Percent_4/univmake_output_BNB_4c_EXT_fix.root",
    "systcalc_modified_4c.conf" ); // "systcalc_unfold_fd_modified.conf" ); // "systcalc_unfold_fd.conf");
  auto& syst = *syst_ptr;
  ...
  for ( size_t sl_idx = 0u; sl_idx < sb.slices_.size(); ++sl_idx ) {
    ...
    for ( const auto& pair : matrix_map ) {
      ...
      if ( color == 6) ++color; // Since we aare skipping total detvar
